  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm px-4">
    <div className="bg-[#141414] border border-[#272727] rounded-3xl p-8 max-w-md w-full space-y-5 shadow-2xl">
      <div className="flex items-center gap-3">
        <span className="w-10 h-10 rounded-2xl bg-red-900/30 border border-red-800 flex items-center justify-center text-red-400 text-xl shrink-0">
          🗑
        </span>
      ...
      <p className="text-gray-400 text-sm leading-relaxed">
        ...
        This cannot be undone and the card will be removed from all category lists.
      </p>
      <div className="flex gap-3 pt-1">
        <button
          onClick={onConfirm}
          className="flex-1 bg-red-600 hover:bg-red-500 active:scale-95 text-white font-semibold py-2.5 rounded-xl transition-all text-sm"
        >
          Yes, Delete
        </button>
        <button
...
export default function EditCard() {
  ...
  const { items, updateItem, removeItem, seedSlugs } = useWorks();
      ...
      <Container>
        ...
        {isStatic && (
          <div className="mb-8 bg-yellow-900/20 border border-yellow-700/40 text-yellow-400 text-sm rounded-2xl px-5 py-3 font-medium">
            ⚠ This is a built-in card. Your changes will be saved in your browser's local storage and will override the original data only for this session.
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <form onSubmit={handleSave} noValidate className="space-y-6">
            ...
            <div>
                ...
                {imgPreview ? (
                  <div className="relative">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center">
                      <span className="text-white text-sm font-semibold bg-black/60 px-4 py-2 rounded-xl">
                        Click to replace
                      </span>
                    </div>
                  </div>
              ...
              {imgPreview && newImgFile && (
                <button
                  type="button"
                  onClick={() => { setImgPreview(item.img || null); setNewImgFile(null); }}
          ...
          <div className="space-y-6">
            ...
            {/* Card preview — matches Linkwork design */}
            ...
            <div className="bg-[#141414] border border-[#272727] rounded-2xl p-5 space-y-3 text-sm">
              ...
              <div className="flex gap-3">
                <span className="text-gray-600 w-24 shrink-0">Slug</span>
                <span className="text-gray-400 font-mono text-xs break-all">/files/{slug}</span>
              </div>
              {form.fileName && (
                <div className="flex gap-3">
